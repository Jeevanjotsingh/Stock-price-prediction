...
if st.button('Submit'):
    try:
        ...
        def predict_weekly_prices(model, scaler, past_data):
            ...
            for _ in range(7):
                ...
                scaled_input_data = scaler.transform(
                    past_data[-300:])  # Increase window size to 300

                # Reshape input data for prediction
                x_test = scaled_input_data.reshape(
                    1, 300, 1)  # Adjust window size
        ...
        # Corrected the reshaping here
        scaler.fit(past_data_array.reshape(-1, 1))

        # Predict stock prices for the next 7 days
        weekly_predictions = predict_weekly_prices(
            model, scaler, past_data_array)
